...
class PNGLoaderTest extends AnyFunSuite {
  private val base = TestUtils.get_test_images_path() + "png";
  test("red pixel") {
    val path = s"$base/red_pixel.png"
    val loader = new PNGLoader(path, MockDecoder);
  ...
  test("white pixel") {
    val path = s"$base/white_pixel.png"
    val loader = new PNGLoader(path, MockDecoder);
    val img = loader.load() match {
      case Left(value) => value;
      case Right(value) => fail(value.msg);
  ...
  test("light blue pixel") {
    val path = s"$base/light_blue_pixel.png"
    val loader = new PNGLoader(path, MockDecoder);
    val img = loader.load() match {
      case Left(value) => value;
      case Right(value) => fail(value.msg);
  ...
  test("non-existent fails") {
    val path = "./src/this/file/does/not/exist.png"
    val loader = new PNGLoader(path, MockDecoder);
    val img = loader.load() match {
